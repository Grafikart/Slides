...
export default makeScene2D(function* (view) {
    ...
    view.add(
        <>
            ...
            >
                <File name="app" />
                <File name="blog" opacity={0} depth={1} />
                <File name="Page.tsx" opacity={0} depth={2} isFile />
                <File name="[id]" opacity={0} depth={2} />
                <File name="Page.tsx" opacity={0} depth={3} isFile />
                <File name="tutoriels" opacity={0} depth={1} />
                <File name="[slug]-[id]" opacity={0} depth={2} />
                <File name="Page.tsx" opacity={0} depth={3} isFile />
            </Rect>
        </>
    )
    ...
    yield* path().text('/blog', 1)

    yield* waitUntil('blog_dir')

    yield* fadeIn(files().childAs<Layout>(1))

    yield* waitUntil('blog_file')

    yield* fadeIn(files().childAs<Layout>(2))

    yield* waitUntil('blog_show')

    yield* path().text('/blog/42', 1)

    yield* waitUntil('blog_id_dir')

    yield* fadeIn(files().childAs<Layout>(3))

    yield* waitUntil('blog_id_file')

    yield* fadeIn(files().childAs<Layout>(4))

    yield* waitUntil('url_pretty')

    yield* path().text('/tutoriels/nextjs-react-1892', 1)
    yield* sequence(.5,
        fadeIn(files().childAs<Layout>(5)),
        fadeIn(files().childAs<Layout>(6)),
        fadeIn(files().childAs<Layout>(7))
    )
...
function* fadeIn(node: Layout, duration = 1) {
    node.margin([0, 50])
    yield* all(
        node.opacity(1, duration),
        node.margin([0, 0], duration)
    )
}
